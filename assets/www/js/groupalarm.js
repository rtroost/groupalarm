var jsgroupalarm = {
	...
	init : function() {
		...
		// ];
		console.log(window.groupids);
		
		for(var i = 0; i < window.groupids.length; i++){
			this.getAll(window.groupids[i]);
		}
	...
	savealarm : function(){
		...
		window.ajax.add({
			...
			jsgroupalarm.createRow({id: msg, hour: hour, min: min, set: false, repDay: [1, 0, 0, 0, 0, 0, 0, 0]}, 'prepend');
		}, function(msg) {
